      ...
      <ul class="List">
        <li v-for="(u, i) in filteredUsers" :key="i">
          <nuxt-link :to="'/competitions/' + $route.params.id + '/' + u.number">
            <avatar v-if="u.userId !== null" :number="users[u.userId].avatar"/>
            <div v-else class="Unknown">?</div>
            <div class="User">
              <el-tag style="float: right">{{ asc ? i + 1 : filteredUsers.length - i }}</el-tag>
              <h4><b>#{{ u.number }}</b> - {{ u.userId !== null ? users[u.userId].name : 'Inconnu' }}</h4>
              <p>
                1) {{ u.times.firstLap || '-'}}
  ...
  computed: {
    filteredUsers () {
      return this.competition ? this.competition.users.filter((u) => {
        let name = u.userId !== null ? this.users[u.userId].name.toLowerCase() : 'inconnu'
